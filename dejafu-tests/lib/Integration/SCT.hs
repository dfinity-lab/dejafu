...
resultsSetTests = toTestList
    ...
    , testCase "Proper results from resultsSet'" $ do
        tested <- resultsSet' defaultWay defaultMemType testAction
        results @=? tested
    , testCase "Proper results from resultsWithSettings" $ do
        tested <-
          resultsSetWithSettings rsSettings testAction
  ...
  where
    ...
    rsSettings =
      let d (Right 1) = Just DiscardResultAndTrace
          d (Right 2) = Just DiscardTrace
          d _ = Nothing
      in set ldiscard (Just d) $
         fromWayAndMemType defaultWay defaultMemType
